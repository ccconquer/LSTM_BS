...
def plot_map(odr_map: Map, ax: plt.Axes = None, scenario_config=None, **kwargs) -> plt.Axes:
    ...
    if kwargs.get("plot_background", False):
        if scenario_config is None:
            ...
            if rescale_factor > 0:
                extent = (0, int(background.size[0] * rescale_factor),
                          -int(background.size[1] * rescale_factor), 0)
                plt.imshow(background, extent=extent)
                plt.savefig('b.jpg',dpi=800)
    ...
    if kwargs.get("plot_buildings", False):
        if scenario_config is None:
            ...
            for building in buildings:
                ...
                plt.plot(*list(zip(*building)), color="yellow", alpha=0.3,linewidth=1)
                plt.fill(*list(zip(*building)), color="grey", alpha=0.5)
                plt.fill(*list(zip(*building)), color="yellow", alpha=0.2, hatch='////\\\\',linewidth=1)
