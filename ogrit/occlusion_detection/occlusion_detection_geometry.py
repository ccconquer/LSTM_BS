...
import sys
sys.path.append('/mnt/home/zhuyanting/LSTM_BS/')
...
class OcclusionDetector2D:
    ...
    def __init__(self, scenario_name: str, episode_idx: int, debug: bool = True, road_occlusions: bool = False):
        ...
        self.episode = self.scenario.load_episode(episode_idx)         # 配置episode，每个episode包含一定量的recording_id
        self.buildings = self.scenario_config.buildings             # 感觉在场景配置文件里json写死的
        self.save_format = "p"  # By default, save the occlusions in a pickle file. 将筛选的盲区存储为pickle文件格式
    ...
    def extract_occlusions(self, save_format="p"):
        ...
        for frame_id, frame in enumerate(episode_frames):  #在循环中使用enumerate()可以同时获取索引和元素值
    ...
    def get_occlusions_ego_by_road(self, ego_position, obstacles):
        ...
        occluded_areas = unary_union([occluded_areas, non_visible_areas])     # 不仅考虑了车辆遮挡导致的盲区，还考虑了超过传感器范围的盲区。（可修正，结合学姐的）

        # Store all the occlusions w.r.t the ego.
        occlusions_by_roads["occlusions"] = self._get_format(occluded_areas)

        # Find what areas in each lane is occluded.  得到盲区和驾驶车道的交集部分
